...
const Example2 = () => {
  // Define countries with their review data
  const countries = [
    {
      color: "#FF5722",
      reviews: {
  ...
  // State to hold the selected time interval
  const [selectedInterval, setSelectedInterval] = useState("Years");

  // Function to get x-values based on the selected time interval
  const getXValues = () => {
  ...
  // Function to get labels for the x-axis
  const getTimeLabels = () => {
    switch (selectedInterval) {
      case "Years":
  ...
  // Calculate review data based on the selected interval
  const getReviewData = () => {
    switch (selectedInterval) {
      case "Years":
        ...
        return weeks.map((week) => {
          ...
          for (let year of years) {
            ...
            if (reviews.length > week - 1) {
              reviewsForWeek.push(reviews[week - 1]); // Adjusting to get week index
            }
          }
        ...
        return days.map((day) => {
          ...
          for (let year of years) {
            ...
            if (reviews.length > day - 1) {
              reviewsForDay.push(reviews[day - 1]); // Adjusting to get day index
            }
          }
  ...
  // Calculate review counts for the current interval
  ...
  // Prepare traces for each country's review data
  const traces = countries.map((country) => ({
    ...
    text: xValues.map((value, index) => 
      `${selectedInterval}: ${value}<br>Rating: ${reviewData[index].toFixed(2)} ⭐<br>Count: ${reviewCounts[index]}`
    ),
    ...
    <div className="flex flex-col z-30 justify-center items-center mt-10">
      <Plot
        ...
        layout={{
          ...
          yaxis: {
            title: "Rating",
            tickvals: [1, 2, 3, 4, 5], // Y-axis values
